...
const WHATSAPP_NUMBER = '5511993225739'; // Sem o + para a API
...
_Sistema de Chamados - Pyramid Diamantados_`;
        ...
        // Dados no formato form-urlencoded correto
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                ...
                try {
                    ...
                    // Se a resposta não for JSON mas o status for 200, assumir sucesso
                    if (res.statusCode === 200) {
                        resolve({
                            success: true,
                            message: '✅ Chamado enviado (resposta em texto)!',
                            response: data
                        });
                    } else {
                        resolve({
...
function testarAPI() {
    console.log('\n🧪 TESTANDO API ULTRAMSG...');
    
    const mensagemTeste = 'Teste do sistema de chamados - Pyramid Diamantados';
    ...
    const dados = {
        ...
        prioridade: '🟡 Teste',
        descricao: 'Mensagem de teste para verificar se a API está funcionando'
    };
    
    enviarWhatsAppUltraMsg(dados)
        .then(resultado => {
    ...
    try {
        ...
        const chamadoCompleto = {
            ...
            credenciais: {
                ...
                token: ULTRAMSG_TOKEN.substring(0, 8) + '...'
            }
        };
...
const server = http.createServer((req, res) => {
    ...
    } else if (pathname === '/teste-api' && req.method === 'GET') {
        // Endpoint para testar a API
        testarAPI();
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ message: 'Teste da API iniciado - verifique o console' }));
...
server.listen(PORT, () => {
    ...
    console.log(`📱 WhatsApp: +55${WHATSAPP_NUMBER}`);
    ...
    console.log('📡 Método: UltraMsg API (form-urlencoded)');
    console.log('🧪 Teste: http://localhost:3000/teste-api');
    console.log('='  .repeat(70));
    console.log('✅ SERVIDOR CORRIGIDO E FUNCIONANDO!');
    console.log('🎯 ENVIO DIRETO PARA WHATSAPP CONFIGURADO!');
    console.log('\n✨ TESTE AGORA O SISTEMA!\n');
    
    // Testar a API automaticamente ao iniciar
    setTimeout(() => {
        console.log('\n🧪 EXECUTANDO TESTE AUTOMÁTICO DA API...');
        testarAPI();
    }, 2000);
});
...
console.log('🔄 Iniciando sistema com correções na API...');